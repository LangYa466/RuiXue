...
class LiquidBounce(inst: Target): TargetStyle("LiquidBounce", inst, true) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
        ...
        Fonts.font35.drawString("距离: ${decimalFormat.format(mc.thePlayer!!.getDistanceToEntityBox(entity))}", 36, 15, getColor(-1).rgb)

        // Draw info
        val playerInfo = mc.netHandler.getPlayerInfo(entity.uniqueID)
        if (playerInfo != null) {
            Fonts.font35.drawString("延迟: ${playerInfo.responseTime.coerceAtLeast(0)}",
