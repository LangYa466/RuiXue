...
import skid.blur.ArrayBlurUtils
@ElementInfo(name = "中文功能列表-字", single = true)
class Arraylist2(x: Double = 1.0, y: Double = 2.0, scale: Float = 1F,
    ...
    private val colorModeValue = ListValue(
        "Color",
        arrayOf("Custom", "Random", "Sky", "CRainbow", "LiquidSlowly", "Fade", "DoubleColor", "Gradinet"),
        "DoubleColor"
    )
    private val blurValue = BoolValue("Blur", false)
    private val blurStrength = FloatValue("Blur-Strength", 0F, 0F, 30F)
    private val shadow = BoolValue("ShadowText", true)
    private val ChineseModuleName = BoolValue("ChineseModuleName", false)
    ...
    private val vAnimation =
        ListValue("VerticalAnimation", arrayOf("None", "LiquidSense", "Slide", "Rise", "Astolfo"), "None")
                    ...
                    when (vAnimation.get()) {
                        "LiquidSense" -> {
                            ...
                            val color4 = RenderUtils.getGradientOffset(
                                ...
                                (kotlin.math.abs(
                                    System.currentTimeMillis() / CustomColor.gradientSpeed.get()
    ...
    fun getModName(mod: Module): String {
        var displayName: String = (if (nameBreak.get()) mod.chinesename else mod.name) + getModTag(mod)
