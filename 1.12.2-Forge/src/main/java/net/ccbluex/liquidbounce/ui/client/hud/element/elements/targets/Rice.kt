...
import me.tiangong.Jello.Stencil
...
import net.ccbluex.liquidbounce.utils.RandomUtils2
...
class Rice(inst: Target): TargetStyle("Rice", inst, true) {
    ...
    override fun drawTarget(entity: IEntityLivingBase) {
                ...
                for (j in 0..(generateAmountValue.get())) {
                    ...
                    particleList.add(Particle(
                        BlendUtils.blendColors(
                            ...
                            if (RandomUtils2.nextBoolean()) RandomUtils.nextFloat(0.5F, 1.0F) else 0F),
        ...
        when (targetInstance.colorModeValue.get().toLowerCase()) {
            "client" -> RoundedUtil.drawGradientRound(5F, 42F, length - maxHealthLength, 48F,
                CustomColor.ra.get(),
    ...
    override fun handleDamage(player: IEntityLivingBase) {
        gotDamaged = true
    }

    override fun handleBlur(player: IEntityLivingBase) {
        val font = Fonts.font40
    ...
    override fun handleShadowCut(player: IEntityLivingBase) = handleBlur(player)

    override fun handleShadow(player: IEntityLivingBase) {
        val font = Fonts.font40
        val name = "Name: ${player.name}"
        val info = "Distance: ${decimalFormat2.format(mc.thePlayer!!.getDistanceToEntityBox(player))}"
        val length = (font.getStringWidth(name).coerceAtLeast(font.getStringWidth(info)).toFloat() + 40F).coerceAtLeast(125F)

        RenderUtils.drawRoundedRect(0F, 0F, 10F + length, 55F, 8F, shadowOpaque.rgb)
    }
