...
object SettingsUtils {
    ...
    fun executeScript(script: String) {
        script.lines().filter { it.isNotEmpty() && !it.startsWith('#') }.forEachIndexed { index, s ->
            ...
            if (args.size <= 1) {
                ClientUtils.displayChatMessage("§7[§3§l参数§7] §cSyntax error at line '$index' in setting script.\n§8§lLine: §7$s")
                return@forEachIndexed
            }

            when (args[0]) {
                "chat" -> ClientUtils.displayChatMessage("§7[§3§l参数§7] §e${translateAlternateColorCodes(StringUtils.toCompleteString(args, 1))}")
                ...
                "load" -> {
                    ...
                    try {
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §7Loading settings from §a§l$url§7...")
                        executeScript(get(url))
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §7Loaded settings from §a§l$url§7.")
                    } catch (e: Exception) {
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §7Failed to load settings from §a§l$url§7.")
                    }
                }

                "targetPlayer", "targetPlayers" -> {
                    EntityUtils.targetPlayer = args[1].equals("true", ignoreCase = true)
                    ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${args[0]}§7 set to §c§l${EntityUtils.targetPlayer}§7.")
                }

                "targetMobs" -> {
                    EntityUtils.targetMobs = args[1].equals("true", ignoreCase = true)
                    ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${args[0]}§7 set to §c§l${EntityUtils.targetMobs}§7.")
                }

                "targetAnimals" -> {
                    EntityUtils.targetAnimals = args[1].equals("true", ignoreCase = true)
                    ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${args[0]}§7 set to §c§l${EntityUtils.targetAnimals}§7.")
                }

                "targetInvisible" -> {
                    EntityUtils.targetInvisible = args[1].equals("true", ignoreCase = true)
                    ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${args[0]}§7 set to §c§l${EntityUtils.targetInvisible}§7.")
                }

                "targetDead" -> {
                    EntityUtils.targetDead = args[1].equals("true", ignoreCase = true)
                    ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${args[0]}§7 set to §c§l${EntityUtils.targetDead}§7.")
                }
                    ...
                    if (module == null) {
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §c功能 §a§l$moduleName§c 不存在!")
                        return@forEachIndexed
                    }

                    if (valueName.equals("toggle", ignoreCase = true)) {
                        module.state = value.equals("true", ignoreCase = true)
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${module.name} §7提示 §c§l${if (module.state) "开启" else "关闭"}§7.")
                        return@forEachIndexed
                    }

                    if (valueName.equals("bind", ignoreCase = true)) {
                        module.keyBind = Keyboard.getKeyIndex(value)
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${module.name} §7被绑定 §c§l${Keyboard.getKeyName(module.keyBind)}§7.")
                        return@forEachIndexed
                    }

                    val moduleValue = module.getValue(valueName)
                    if (moduleValue == null) {
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §cValue §a§l$valueName§c 找不到功能 §a§l$moduleName§c.")
                        return@forEachIndexed
                    }
                    ...
                    try {
                        ...
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${module.name}§7 value §8§l${moduleValue.name}§7 修改至 §c§l$value§7.")
                    } catch (e: Exception) {
                        ClientUtils.displayChatMessage("§7[§3§l参数§7] §a§l${e.javaClass.name}§7(${e.message}) §c设置失败 §a§l$value§c to §a§l${moduleValue.name}§c in §a§l${module.name}§c.")
                    }
                }
            }
        }
