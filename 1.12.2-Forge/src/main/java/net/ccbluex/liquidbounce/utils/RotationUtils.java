...
public final class RotationUtils extends MinecraftInstance implements Listenable {
    ...
    public static VecRotation searchdown(final IAxisAlignedBB bb, final boolean outborder, final boolean random,
                                         final boolean predict, final boolean throughWalls, final float distance) {
        if (outborder) {
            final WVec3 vec3 = new WVec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * (x * 0.3 + 1.0), bb.getMinY() + (bb.getMaxY() - bb.getMinY()) * (y * 0.3 + 1.0), bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * (z * 0.3 + 1.0));
            return new VecRotation(vec3, toRotation(vec3, predict));
        ...
        double zDist = 0.0D;
        VecRotation vecRotation = null;
        xMin = 0.10D; xMax = 0.70D;xDist = 0.05D;
        yMin = 0.10D; yMax = 0.32D;yDist = 0.03D;
        zMin = 0.10D; zMax = 0.70D;zDist = 0.05D;
        for(double xSearch = xMin; xSearch < xMax; xSearch += xDist) {
            for (double ySearch = yMin; ySearch < yMax; ySearch += yDist) {
                for (double zSearch = zMin; zSearch < zMax; zSearch += zDist) {
                    final WVec3 vec3 = new WVec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * xSearch, bb.getMinY() + (bb.getMaxY() - bb.getMinY()) * ySearch, bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * zSearch);
    ...
    public static VecRotation searchHead(final IAxisAlignedBB bb, final boolean outborder, final boolean random,
                                         final boolean predict, final boolean throughWalls, final float distance) {
        if (outborder) {
            final WVec3 vec3 = new WVec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * (x * 0.3 + 1.0), bb.getMinY() + (bb.getMaxY() - bb.getMinY()) * (y * 0.3 + 1.0), bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * (z * 0.3 + 1.0));
            return new VecRotation(vec3, toRotation(vec3, predict));
        ...
        double zDist = 0.0D;
        VecRotation vecRotation = null;
        xMin = 0.50D; xMax = 0.60D; xDist = 0.01D;
        yMin = 0.88D; yMax = 1.00D; yDist = 0.01D;
        zMin = 0.50D; zMax = 0.60D; zDist = 0.01D;
        for(double xSearch = xMin; xSearch < xMax; xSearch += xDist) {
            for (double ySearch = yMin; ySearch < yMax; ySearch += yDist) {
                for (double zSearch = zMin; zSearch < zMax; zSearch += zDist) {
                    final WVec3 vec3 = new WVec3(bb.getMinX() + (bb.getMaxX() - bb.getMinX()) * xSearch, bb.getMinY() + (bb.getMaxY() - bb.getMinY()) * ySearch, bb.getMinZ() + (bb.getMaxZ() - bb.getMinZ()) * zSearch);
    ...
    public static void setRotation(EntityLivingBase player, float targetPitch, float targetYaw, float serverPitch, float serverYaw) {
        // 客户端设置玩家的俯仰角度和偏航角度
        player.rotationPitch = targetPitch;
        player.rotationYaw = targetYaw;

        // 服务器设置玩家的俯仰角度和偏航角度
        player.prevRotationYawHead = serverPitch;
