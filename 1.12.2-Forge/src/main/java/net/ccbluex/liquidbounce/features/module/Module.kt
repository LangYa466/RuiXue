...
import me.tiangong.Jello.Translate
...
open class Module() : MinecraftInstance(), Listenable {
    ...
    val moduleTranslate = net.ccbluex.liquidbounce.utils.Translate(0F, 0F)
    val tab = net.ccbluex.liquidbounce.utils.Translate(0f, 0f)
    // Module information
        ...
        set(value) {
            ...
            if (!LiquidBounce.isStarting) {
                if(value){
                    ...
                    LiquidBounce.hud.addNotification(Notification("功能提示",
                        "$chinesename 已开启", NotifyType.SUCCESS))
                }else{
                    mc.soundHandler.playSound("random.click", 1F)
                    LiquidBounce.hud.addNotification(Notification("功能提示",
                        "$chinesename 已关闭", NotifyType.ERROR))
                }
            }
