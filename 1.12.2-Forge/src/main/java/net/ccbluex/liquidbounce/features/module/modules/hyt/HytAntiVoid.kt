...
@ModuleInfo(name = "HytAntiVoid",  description = "./Test but for fdpcn betstantivoid",chinesename = "花雨庭虚空回弹", category = ModuleCategory.VULGAR)
class HytAntiVoid : Module() {
    ...
    fun onRevPacket(e: PacketEvent) {
        if (e.packet.unwrap() is SPacketPlayerPosLook && packets.size > 1) {
            debug("[花雨庭虚空回弹] Pullbacks Detected, clear packets list!")
            packets.clear()
        }
    }
