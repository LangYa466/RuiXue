...
import net.ccbluex.liquidbounce.utils.item.InventoryUtils
...
class InvManager : Module() {
    ...
    private val sortSlot2Value = ListValue("SortSlot-2", items, "Block")
    private val sortSlot3Value = ListValue("SortSlot-3", items, "Gapple")
    private val sortSlot4Value = ListValue("SortSlot-4", items, "Bow")
    private val sortSlot5Value = ListValue("SortSlot-5", items, "None")
    private val sortSlot6Value = ListValue("SortSlot-6", items, "None")
    private val sortSlot7Value = ListValue("SortSlot-7", items, "None")
    private val sortSlot8Value = ListValue("SortSlot-8", items, "None")
    private val sortSlot9Value = ListValue("SortSlot-9", items, "None")
    ...
    fun isUseful(itemStack: IItemStack, slot: Int): Boolean {
        return try {
            ...
            } else (classProvider.isItemFood(item) && classProvider.isItemAppleGold(item)) || itemStack.unlocalizedName == "item.arrow" || itemStack.unlocalizedName == "item.slimeball" || (classProvider.isItemBlock(item) && !InventoryUtils.SKYWAR_BLOCK_BLACKLIST.contains(item!!.asItemBlock().block)) || classProvider.isItemBed(item) || classProvider.isItemPotion(
                item
