...
@ModuleInfo(name = "BowAimbot", description = "Automatically aims at players when using a bow.",chinesename = "弓箭瞄准", category = ModuleCategory.COMBAT)
class BowAimbot : Module() {

    private val silentValue = BoolValue("Silent-静默", true)
    private val predictValue = BoolValue("Predict-预判", true)
    private val throughWallsValue = BoolValue("ThroughWalls-穿墙", false)
    private val predictSizeValue = FloatValue("PredictSize-预判速度", 2F, 0.1F, 5F)
    private val priorityValue = ListValue("Priority-优先权", arrayOf("Health", "Distance", "Direction"), "Direction")
    private val markValue = BoolValue("Mark-标记", true)
