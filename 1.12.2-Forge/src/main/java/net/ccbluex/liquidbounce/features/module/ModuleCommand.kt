...
class ModuleCommand(val module: Module, val values: List<Value<*>> = module.values) :
    ...
    override fun execute(args: Array<String>) {
        ...
        if (value is BoolValue) {
            ...
            chat("§7${module.name} §8${args[1]}§7 提示 ${if (newValue) "§8开启§7" else "§8关闭§7" + "."}")
            playEdit()
        } else {
            if (args.size < 3) {
                if (value is IntegerValue || value is FloatValue || value is TextValue)
                    chatSyntax("$moduleName ${args[1].toLowerCase()} <参数>")
            ...
            try {
                when (value) {
                    is BlockValue -> {
                        ...
                        value.set(id)
                        chat("§7${module.name} §8${args[1].toLowerCase()}§7 修改至 §8${BlockUtils.getBlockName(id)}§7.")
                        playEdit()
                        return
                    }
