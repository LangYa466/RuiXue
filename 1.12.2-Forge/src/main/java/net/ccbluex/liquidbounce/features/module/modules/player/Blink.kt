...
import net.ccbluex.liquidbounce.utils.PingSpoofUtils
...
@ModuleInfo(name = "Blink", description = "Blink Fake Player", chinesename = "瞬移", category = ModuleCategory.PLAYER)
class Blink : Module() {
    ...
    private val pulseDelayValue = IntegerValue("PulseDelay", 500, 500, 5000)
    private val bypassInfiniteValue = BoolValue("BypassInfinite",false)
    ...
    override fun onEnable() {
        ...
        PingSpoofUtils.setPingSpoof(true)
        PingSpoofUtils.setDelay(999999999,999999999)
    ...
    override fun onDisable() {
        ...
        packetQueue.clear()
        inBus.clear()
        PingSpoofUtils.setPingSpoof(false)
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if ((classProvider.isCPacketKeepAlive(packet) || classProvider.isCPacketClientStatus(packet)) && !(mc.thePlayer!!.isDead || mc.thePlayer!!.health <= 0) && !packetQueue.containsKey(packet)) {
            event.cancelEvent()

            synchronized(packetQueue) {
                packetQueue[packet] = System.currentTimeMillis() + TimeUtils.randomDelay(500, 1000)
            }
        }
        ...
        if (classProvider.isCPacketPlayerPosition(packet) || classProvider.isCPacketPlayerPosLook(packet) ||
            ...
            classProvider.isCPacketEntityAction(packet) || classProvider.isCPacketUseEntity(packet) || (bypassInfiniteValue.get() && classProvider.isCPacketConfirmTransaction(packet))) {
            event.cancelEvent()
            packets.add(packet)
        }
        if (packet.unwrap() is SPacketConfirmTransaction && bypassInfiniteValue.get()) {
            event.cancelEvent()
