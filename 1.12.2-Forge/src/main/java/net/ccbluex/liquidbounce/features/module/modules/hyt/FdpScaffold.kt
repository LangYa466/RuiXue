...
@ModuleInfo(name = "FdpScaffold", description = "Scaffold" ,chinesename = "FDP自动搭路", category = ModuleCategory.VULGAR)
    ...
    fun onUpdate(event: UpdateEvent) {
        ...
        if (towerStatus || mc.thePlayer!!.isCollidedHorizontally) {
            ...
            when(sameYValue.get().toLowerCase()){
                ...
                "autojump" -> {
                    canSameY = true
                    if (mc.thePlayer!!.onGround && isMoving) mc.thePlayer!!.jump()
                }
                "whenspeed" -> {
