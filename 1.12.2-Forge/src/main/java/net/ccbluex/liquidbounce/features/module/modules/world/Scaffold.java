...
@ModuleInfo(
        name = "Scaffold",
        description = "Fix",
        category = ModuleCategory.WORLD,chinesename = "自动搭路",
...
public class Scaffold extends Module {
    ...
    public final BoolValue onlyairsprint = new BoolValue("Onlyairsprint", true);
    ...
    // Rotations
    private final ListValue rotationModeValue = new ListValue("RotationMode", new String[]{"Normal", "瑞雪", "Static", "StaticPitch", "StaticYaw", "Off"}, "Normal");
    ...
    private final ListValue sameY2Value = new ListValue("FDPSameY", new String[]{"Simple", "AutoJump", "WhenSpeed", "OFF"}, "WhenSpeed");
    ...
    public void onUpdate(final UpdateEvent event) {
        if (sameY2Value.get().equalsIgnoreCase("simple")) {
            canSameY = true;
        } else if (sameY2Value.get().equalsIgnoreCase("autojump")) {
            canSameY = true;
            if (mc.getThePlayer().getOnGround() && MovementUtils.isMoving())
                mc.getThePlayer().jump();
        } else if (sameY2Value.get().equalsIgnoreCase("whenspeed")) {
            canSameY = LiquidBounce.moduleManager.getModule(Speed.class).getState();
        } else {
            canSameY = false;
        }
        if (mc.getThePlayer().getOnGround()) {
            lastGroundY = (int) mc.getThePlayer().getPosY();
        }
        getBestBlocks();
        mc.getTimer().setTimerSpeed(timerValue.get());
        ...
        if (sprintValue.get() && onlyairsprint.get()) {
            if (!mc.getGameSettings().isKeyDown(mc.getGameSettings().getKeyBindSprint()) && mc.getThePlayer().isAirBorne()) {
                mc.getGameSettings().getKeyBindSprint().setPressed(false);
            }
            if (mc.getGameSettings().isKeyDown(mc.getGameSettings().getKeyBindSprint()) && mc.getThePlayer().isAirBorne()) {
                mc.getGameSettings().getKeyBindSprint().setPressed(true);
            }
            if (mc.getGameSettings().getKeyBindSprint().isKeyDown() && mc.getThePlayer().isAirBorne()) {
                mc.getThePlayer().setSprinting(true);
            }
            if (!mc.getGameSettings().getKeyBindSprint().isKeyDown() && mc.getThePlayer().isAirBorne()) {
                mc.getThePlayer().setSprinting(false);
...
//        }
    ...
    public void onMotion(final MotionEvent event) {
        ...
        //fixed liquidbounce scaffold slow
        if (slowValue.get()) {
                ...
                for (double ySearch = 0.5D - (yRV / 2); ySearch <= 0.5D + (yRV / 2); ySearch += ySSV) {
                    for (double zSearch = 0.5D - (xzRV / 2); zSearch <= 0.5D + (xzRV / 2); zSearch += xzSSV) {
                        ...
                        for (int i = 0; i < (staticYawMode ? 2 : 1); i++) {
                            ...
                            if (!rotationModeValue.get().equalsIgnoreCase("瑞雪"))
                                RotationUtils.setRotation(mc2.player,mc.getThePlayer().getRotationPitch(),mc.getThePlayer().getRotationYaw(),180.0f,95.0f);
