...
@ModuleInfo(name = "LongJump", description = "Allows you to jump further.",chinesename = "长跳", category = ModuleCategory.MOVEMENT)
class LongJump : Module() {
    private val modeValue = ListValue("Mode", arrayOf("NCP", "Hyt", "AACv1", "AACv2", "AACv3", "Mineplex", "Mineplex2", "Mineplex3"), "NCP")
    ...
    val VERTICAL_SPEED = 0.03147
    val HORIZONTAL_SPEED_MULTIPLIER = 1.0114514f
    ...
    fun onUpdate(event: UpdateEvent?) {
        ...
        if (LadderJump.jumped)
            strafe(MovementUtils.speed * 1.08f)
        ...
        if (jumped) {
            ...
            run {
                when (mode.toLowerCase()) {
                    "ncp" -> {
                        strafe(MovementUtils.speed * if (canBoost) ncpBoostValue.get() else 1f)
                        canBoost = false
                    }
                    "aacv1" -> {
                    ...
                    player.motionY += this.VERTICAL_SPEED
                        var  playerSpeed
                        : Double = Math.sqrt(player.motionX * player.motionX + player.motionZ * player.motionZ)
                        var  speed
                        : Float = (playerSpeed * this.HORIZONTAL_SPEED_MULTIPLIER).toFloat()
                    strafe(speed)
