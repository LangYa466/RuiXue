...
@ModuleInfo(name = "AntiBot", description = "Prevents KillAura from attacking AntiCheat bots.",chinesename = "防假人", category = ModuleCategory.MISC)
class AntiBot : Module() {
    private val tabValue = BoolValue("Tab-计分板存在", true)
    private val tabModeValue = ListValue("TabMode-计分板模式", arrayOf("Equals", "Contains"), "Contains")
    private val entityIDValue = BoolValue("EntityID-实体ID", true)
    private val colorValue = BoolValue("Color-颜色", false)
    private val livingTimeValue = BoolValue("LivingTime-进入服务器时间", false)
    private val livingTimeTicksValue = IntegerValue("LivingTimeTicks-最小多少时间", 40, 1, 200)
    private val groundValue = BoolValue("Ground-平地", true)
    private val airValue = BoolValue("Air-空中", false)
    private val invalidGroundValue = BoolValue("InvalidGround-无效平地", true)
    private val swingValue = BoolValue("Swing-挥手", false)
    private val healthValue = BoolValue("Health-血量", false)
    private val derpValue = BoolValue("Derp-我不知道啊", true)
    private val wasInvisibleValue = BoolValue("WasInvisible-隐身", false)
    private val armorValue = BoolValue("Armor-装备", false)
    private val pingValue = BoolValue("Ping-延迟", false)
    private val needHitValue = BoolValue("NeedHit-是否能攻击", false)
    private val duplicateInWorldValue = BoolValue("DuplicateInWorld-我不知道啊", false)
    private val duplicateInTabValue = BoolValue("DuplicateInTab-我不知道啊", false)
