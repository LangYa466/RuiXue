...
@ModuleInfo(name = "MemoryFix",category = ModuleCategory.MISC,chinesename = "内存溢出修复", description = "idk")
public class MemoryFix extends Module {
    ...
    public void onTick(TickEvent event) {
        ...
        if (timer.hasReached(delay.get() * 1000.0D) && limit.get() <= (double)pct) {
                System.gc();
            ClientUtils.displayChatMessage(LiquidBounce.CLIENT_NAME + " | 内存清理成功");
        }
    }
    @EventTarget
