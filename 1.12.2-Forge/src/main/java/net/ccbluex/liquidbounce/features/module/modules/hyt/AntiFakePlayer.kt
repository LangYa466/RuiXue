...
@ModuleInfo(name = "AntiFakePlayer", description = "Made and Fixed by RyF with ❤", chinesename = "花雨庭防假人", category = ModuleCategory.VULGAR)
class AntiFakePlayer : Module() {
    ...
    private val showMyKillDeathChatValue =
        BoolValue("ShowMyKillDeathChat", false)
    ...
    private val chatHiddenValue = BoolValue("IgnoredChat", false)
    ...
    private val autoSwitchDelay =
        IntegerValue("AutoSwitchDelay", 3000, 1500, 7000)
    ...
    private val sTHideLogStyle =
        BoolValue("HideLogStyle", false)

    // 隐藏无敌人数量开关
    private val sTHideBotsCounter =
        BoolValue("HideBotsCounter", false)
    // 设置
    ...
    private val sTHideAllAutoSwitcher =
        BoolValue("HideAllAutoSwitcher", false)
    ...
    fun onPacket(event: PacketEvent) {
        ...
        if (packet is SPacketChat && packet.chatComponent.unformattedText.contains(
            ...
            if (hideAntiGetNameValue.get()) {
                event.cancelEvent()
                if (chatHiddenValue.get()) displayChatMessage("§b$CLIENT_NAME §7» §c隐藏了无敌人消息。")
            }
        }
        ...
        if (packet is SPacketChat) {
            ...
            if (modeValue.get().toLowerCase() == "kitbattle" && chat.startsWith("花雨庭")) {
                ...
                if (kitHideKillCoinValue.get() && (chat.contains("花雨庭 >>你消灭") && chat.contains("% 的伤害并且获得了") && chat.contains(
                        "硬币!"
                    ) || chat.contains("你的 coins 被修正为") || chat.contains("搂c搂l您本日获得的 搂7搂l银币搂c搂l 已达上限,接下来的游戏将不再获得。"))
                ) {
                    event.cancelEvent()
                }
    ...
    private fun playerDeathAction(name: String, cd: Long) {
        if (name == null) return
        bots++
        if (!fileManager.friendsConfig.isFriend(name!!)) {
        ...
        when (mode.toLowerCase()) {
            "add" -> {
                when (logStyleValue.get().toLowerCase()) {
                    "ryfnew" -> displayChatMessage("§b$CLIENT_NAME §7» §a添加§f 花雨庭假人 §7-> §e$name")
                    "fdpantibot" -> displayChatMessage("§7[§cAntiBot§7] §f添加 §7$name§f 因为它是一个假人.")
                    "fdpchat" -> displayChatMessage("§f[§c!§f] §b${CLIENT_NAME} §7>> §a添加 §6花雨庭 假人§f[$name§f]§6.")
                    "leave" -> displayChatMessage("§b${CLIENT_NAME} §8[§e警告§8] §6添加无敌人: $name")
                    "special" -> displayChatMessage("§8[§d${CLIENT_NAME}§8] §a$name§d被§bRyF§d吃掉啦! §bCiallo(∠・ω< )⌒☆")
                    "wawa1" -> displayChatMessage("§6${CLIENT_NAME} §7=> §添加 假人 §7$name§f.")
                    "wawa2" -> displayChatMessage("§6${CLIENT_NAME} §7» §f玩家死亡: §7$name")
                    "nullclient" -> displayChatMessage("§7[§c防假人§7] §添加一个假人(§7$name§f)")
                    "normal" -> displayChatMessage("§7[§6${CLIENT_NAME}§7] §f添加花雨庭假人: §7$name")
                    "windx" -> displayChatMessage("§7[§c!§7] §b瑞雪 §a客户端 §7=> §a添加 §f一个 假人(§7$name§f)")
                    "old" -> displayChatMessage("§8[§c§l${CLIENT_NAME}§8] §d添加无敌人：§7$name")
                    "arene" -> displayChatMessage("§7[§f${CLIENT_NAME}§7] §f添加一个假人(§7$name§f)")
                }
            }

            "remove" -> {
                when (logStyleValue.get().toLowerCase()) {
                    "ryfnew" -> displayChatMessage("§b$CLIENT_NAME §7» §c移除§f 花雨庭假人 §7-> §e$name")
                    "fdpantibot" -> displayChatMessage("§7[§cAntiBot§7] §f移除 §7$name§f 因为复活")
                    "fdpchat" -> displayChatMessage("§f[§c!§f] §b${CLIENT_NAME} §7>> §c移除 §6花雨庭假人§f[$name§f]§6.")
                    "leave" -> displayChatMessage("§b${CLIENT_NAME} §8[§e警告§8] §6删除无敌人: $name")
                    "special" -> displayChatMessage("§8[§d${CLIENT_NAME}§8] §a$name§d被§bRyF§d吐出来咯~ §bCiallo(∠・ω< )⌒☆")
                    "wawa1" -> displayChatMessage("§6${CLIENT_NAME} §7=> §f移除 假人 §7$name§f.")
                    "wawa2" -> displayChatMessage("§6${CLIENT_NAME} §7» §f玩家重生: §7$name")
                    "nullclient" -> displayChatMessage("§7[§cAntiBots§7] §f移除 一个假人(§7$name§f)")
                    "normal" -> displayChatMessage("§7[§6${CLIENT_NAME}§7] §f移除 花雨庭假人: §7$name")
                    "windx" -> displayChatMessage("§7[§c!§7] §b瑞雪 §a客户端 §7=> §c移除 §f一个假人(§7$name§f)")
                    "old" -> displayChatMessage("§8[§c§l${CLIENT_NAME}§8] §d删除无敌人：§7$name")
                    "arene" -> displayChatMessage("§7[§f${CLIENT_NAME}§7] §fDel 一个假人(§7$name§f)")
                }
            }
        }
    }
    ...
    private fun tagReturner(): String {
        ...
        if (sTHideAll.get()) {
            ...
            if (!sTHideBotsCounter.get()) {
                if (tag != "") tag = "$tag, 假人: $bots" else "假人: $bots"
            } else {
            ...
            if (autoSwitchLogger.get() && !sTHideAllAutoSwitcher.get()) {
                if (tag != "" && !sTHideAutoSwitcherState.get()) tag =
                    "$tag, 自动选择:${if (autoSwitchLogger.get()) "开启" else "关闭"}"
                if (sTHideAutoSwitcherState.get() && sTHideAutoSwitcherStateSetting.get()
                        .toLowerCase() == "onlyshowstate"
                ) {
                    tag =
                        if (tag != "") "$tag, ${if (autoSwitchLogger.get()) "开启" else "关闭"}" else if (autoSwitchLogger.get()) "开启" else "关闭"
                }
