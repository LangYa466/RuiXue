...
@ModuleInfo(name = "Crosshair", description = "Cancels blindness effects.", category = ModuleCategory.RENDER)
class Crosshair : Module() {
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        val target = aura.target/* ?: RaycastUtils.raycastEntity(Reach.hitReach.toDouble()) {
    ...
    private val crosshairColor: Color
        get() =
            when (colorModeValue.get().toLowerCase()) {
                ...
                "rainbow" -> Colors.getColorFromInt(Colors.getRainbow(500, 0))
                else -> Color.WHITE
            }
}