...
object BlurUtils : MinecraftInstance() {
    ...
    private val shaderGroup = ShaderGroup(mc2.textureManager, mc2.resourceManager, mc2.getFramebuffer(), ResourceLocation("shaders/post/blurArea.json"))
    ...
    private fun setValues(strength: Float, x: Float, y: Float, w: Float, h: Float, width: Float, height: Float, force: Boolean = false) {
        ...
        for (i in 0..1) {
            (shaderGroup.listShaders[i] as Shader).shaderManager.getShaderUniform("Radius")!!
                .set(strength)
            (shaderGroup.listShaders[i] as Shader).shaderManager.getShaderUniform("BlurXY")!![x] =
                height - y - h
            (shaderGroup.listShaders[i] as Shader).shaderManager.getShaderUniform("BlurCoord")!![w] =
                h
        }
    }
    ...
    fun blur(posX: Float, posY: Float, posXEnd: Float, posYEnd: Float, blurStrength: Float, displayClipMask: Boolean, triggerMethod: () -> Unit) {
        ...
        val sc = classProvider.createScaledResolution(mc)
        ...
        bufferbuilder.pos(0.0, height.toDouble(), 0.0).tex(0.0, 0.0).color(255, 255, 255, 255).endVertex()
        bufferbuilder.pos(width.toDouble(), height.toDouble(), 0.0).tex(f2, 0.0).color(255, 255, 255, 255).endVertex()
        bufferbuilder.pos(width.toDouble(), 0.0, 0.0).tex(f2, f3).color(255, 255, 255, 255).endVertex()
